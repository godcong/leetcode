...
func questionData(code *Code, now time.Time) error {
...
func dailyQuestionRecords(now time.Time) (Code, error) {
...
func questionOfToday(code *Code) error {
...
func GetCode() (*Code, error) {
	t := time.Now()
	records, err := dailyQuestionRecords(t)
	if err != nil {
		return nil, fmt.Errorf("dailyQuestionRecords:%v", err)
	}
	if err := questionOfToday(&records); err != nil {
		return nil, fmt.Errorf("questionOfToday:%v", err)
	}
	if err := questionData(&records, t); err != nil {
