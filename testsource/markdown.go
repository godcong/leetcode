...
func markdownHTML() {
	...
	md := []byte(`<p>You are given an array of integers <code>stones</code> where <code>stones[i]</code> is the weight of the <code>i<sup>th</sup></code> stone.</p>
...
`)
	...
	file, err := os.OpenFile("test.md", os.O_CREATE|os.O_SYNC|os.O_RDWR, 0755)
	if err != nil {
		return
	}


	godown.Convert(file, r, nil)
}
