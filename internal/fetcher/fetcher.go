...
// Code is an alias for types.Code
type Code = types.Code

// Payload for GraphQL requests
type Payload struct {
...
// Query handles LeetCode API queries
type Query struct {
	cookie string
	c      *http.Client
}
...
// GetCode fetches problem data from LeetCode API
func (q *Query) GetCode(code *Code, name string) error {
	logger.Debug("GetCode called", "name", name)

	if name == "" {
		logger.Debug("Fetching daily question")
	...
	tempCode, err := q.getNumberCode(parsedInt)
	if err != nil {
		logger.Error("getNumberCode failed", err, "number", parsedInt)
		return fmt.Errorf("getNumberCode: %v", err)
	}
	...
	logger.Debug("getNumberCode result",
		"number", tempCode.Result.Number,
		"slug", tempCode.Result.Slug,
		"hasData", tempCode.Data.ProblemSetQuestionList.Total > 0)

	if tempCode.Result.Slug == "" {
		logger.Error("getNumberCode returned empty slug",
			fmt.Errorf("empty slug"),
			"number", parsedInt,
			"result", tempCode.Result)
		return fmt.Errorf("failed to get problem slug")
	}

	// Fetch daily question records to get date info
	logger.Debug("Fetching daily question records")
	dailyCode, err := q.dailyQuestionRecords(time.Now())
	if err != nil {
		logger.Warn("Failed to fetch daily question records", err)
	} else {
		// Copy daily records to code object
		code.Data.DailyQuestionRecords = dailyCode.Data.DailyQuestionRecords
		logger.Debug("Daily question records fetched", "count", len(code.Data.DailyQuestionRecords))
	}
		
	// Then fetch full data using slug
	logger.Debug("Fetching full problem data", "slug", tempCode.Result.Slug)
	err = q.questionData(code, tempCode.Result.Slug)
	if err != nil {
		logger.Error("questionData failed", err, "slug", tempCode.Result.Slug)
		return err
	}
		
	// Set the date from daily records if this problem matches today's daily question
	if len(code.Data.DailyQuestionRecords) > 0 {
		for _, record := range code.Data.DailyQuestionRecords {
			if record.Question.QuestionTitleSlug == code.Result.Slug {
				// Found match, create TodayRecord with the date
				code.Data.TodayRecord = append(code.Data.TodayRecord, types.TodayRecord{
					Date: record.Date,
				})
				logger.Debug("Matched daily question", "date", record.Date, "slug", code.Result.Slug)
				break
			}
		}
	}

	logger.Debug("GetCode completed successfully",
...
func (q *Query) getDailyQuestion(code *Code) error {
	// TODO: Implement daily question fetch
	return nil
}

func (q Query) questionData(code *Code, name string) error {
	logger.Debug("questionData called", "name", name)
	...
	//   --compressed
	logger.Debug("questionData processing", "name", name)

	current := name
	logger.Debug("Using titleSlug", "current", current)
	if current == "" {
  ...
  question(titleSlug: $titleSlug) {
	...
	if code.Data.Question.QuestionFrontendID != "" {
		code.Result.Number = code.Data.Question.QuestionFrontendID
		code.Result.Slug = code.Data.Question.TitleSlug
		logger.Debug("Set result metadata", "number", code.Result.Number, "slug", code.Result.Slug)
	} else {
		logger.Warn("Question data missing, using input name", "name", name)
		code.Result.Number = name
		code.Result.Slug = name
	}
...
func (q Query) DailyQuestionRecords(now time.Time) (Code, error) {
	return q.dailyQuestionRecords(now)
}
...
func (q Query) getNumberCode(codeNum int64) (*Code, error) {
	logger.Debug("Fetching problem by number", "number", codeNum)
	skip := int(codeNum / 50 * 50)
	limit := 50
	logger.Debug("Calculated pagination", "skip", skip, "limit", limit)

	records, err := q.problemSetQuestionList(skip, limit)
	if err != nil {
		logger.Error("problemSetQuestionList failed", err)
		return nil, err
	}
	logger.Debug("problemSetQuestionList returned",
		"total", records.Data.ProblemSetQuestionList.Total,
		"questionsCount", len(records.Data.ProblemSetQuestionList.Questions))

	for _, question := range records.Data.ProblemSetQuestionList.Questions {
		logger.Debug("Checking question",
			"frontendId", question.FrontendQuestionID,
			"slug", question.TitleSlug)
		i, err := strconv.ParseInt(question.FrontendQuestionID, 10, 32)
		if err != nil {
			logger.Debug("Failed to parse question ID", "id", question.FrontendQuestionID, "error", err)
			continue
		}
		if codeNum == i {
			logger.Debug("Found matching problem", "number", codeNum, "slug", question.TitleSlug)
	...
	logger.Debug("getNumberCode completed",
		"resultNumber", records.Result.Number,
		"resultSlug", records.Result.Slug)
	return &records, nil
}
