...
- [LeetCode](https://leetcode-cn.com/problems/design-task-manager/)
...
- `TaskManager(vector>& tasks)` 初始化任务管理器，初始化的数组格式为 `[userId, taskId, priority]` ，表示给 `userId` 添加一个优先级为 `priority` 的任务 `taskId` 。
